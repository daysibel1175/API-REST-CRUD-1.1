      ...
      .catch((error) =>
        res
          ...
          .json({
            message:
              error +
              "O Id não foi encontrado ou ainda precisa inserir esses dados",
          })
      );
  } catch (error) {
    res.status(500);
...
// Read - leitura de um dado especifico usando o parametro email
router.get("/usuario/read-email/:email", (req, res) => {
  try {
    const { email } = req.params;
    if (!email) return res.status(404).send({ error: "Usuario não existe" });
      ...
      .catch((error) =>
        res
          ...
          .json({
            message:
              error +
              "O email não foi encontrado ou ainda precisa inserir esses dados",
          })
      );
  } catch (error) {
    res.status(500);
...
router.patch("/usuario/update/:id", (req, res) => {
  try {
    ...
    Usuario.updateOne({ _id: id }, { $set: dados })
      .then((data) =>
        res.status(200).json("Seus dados foram atualizados com sucesso")
      )
      .catch((error) =>
        res
          .status(304)
          .json({
            message:
              error +
              "Nao foi possivel fazer atualizacao dos dados! O ID não existe ou ainda nao foi inserido",
          })
      );
      ...
      .catch((error) =>
        res
          ...
          .json({
            message:
              error +
              "Error ao deletar os dados selecionados, ID ainda nao foi inserido ou ID invalido",
          })
      );
  } catch (error) {
    res.status(500);
