...
router.post("/trilhas", async (req, res) => {
  try {
    ...
    if (!tipo_de_trilha || typeof tipo_de_trilha !== "string") return res.status(400).json({ message: "tipo_de_trilha é obrigatório" });
...
router.get("/trilhas", async (req, res) => {
  try {
    const items = await Trilha.find().populate({ path: "guia", select: "nome" }).populate({ path: "grupo" });
...
router.get("/trilhas/:id", async (req, res) => {
  try {
    const item = await Trilha.findById(req.params.id).populate("guia").populate("grupo");
    if (!item) return res.status(404).json({ message: "Trilha não encontrada" });
...
router.patch("/trilhas/:id", async (req, res) => {
  try {
    const item = await Trilha.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ message: "Trilha não encontrada" });
    return res.status(200).json(item);
  } catch (error) {
    return res.status(500).json({ message: "Erro interno", error: String(error) });
...
router.delete("/trilhas/:id", async (req, res) => {
  try {
    ...
    if (!result) return res.status(404).json({ message: "Trilha não encontrada" });
