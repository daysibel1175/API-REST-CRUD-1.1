...
router.get("/guias", async (req, res) => {
  try {
    const items = await Guia.find().populate({ path: "trilha", select: "nome" }).populate({ path: "grupo" });
...
router.get("/guias/:id", async (req, res) => {
  try {
    const item = await Guia.findById(req.params.id).populate("grupo").populate({ path: "trilha", select: "nome" });
...
router.patch("/guias/:id", async (req, res) => {
  try {
    const item = await Guia.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!item) return res.status(404).json({ message: "Guia não encontrado" });
    return res.status(200).json(item);
  } catch (error) {
    return res.status(500).json({ message: "Erro interno", error: String(error) });
...
router.delete("/guias/:id", async (req, res) => {
  try {
    ...
    if (!result) return res.status(404).json({ message: "Guia não encontrado" });
